    ...
    case REQUEST_CREATE_AGREEMENT:
      ...
      for (let i = 0; i < state.pledges.length; i += 1) {
        if (state.pledges[i].deadline > action.newPledge.deadline) {
          console.log('action.newPledge.deadline', action.newPledge.deadline);
          console.log('state.pledges[i].deadline', state.pledges[i].deadline);
          insertBeforeThis = i;
          console.log('insertBeforeThis', insertBeforeThis);
          break;
        }
      }
